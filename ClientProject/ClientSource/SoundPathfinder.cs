...
namespace SoundproofWalls
{
    ...
    public static class SoundPathfinder
    {
        ...
        private static float GetGapTraversalPenalty(Gap gap, bool ignoreDoors = false)
        {
            ...
            float penalty = 0.0f;
            bool impassableStructure = false;
            ...
            {
                impassableStructure = true;
            }

            if (impassableStructure) return float.MaxValue; // Cannot pass through structure
        ...
        public static List<PathfindingResult> FindShortestPaths(Vector2 sourcePos, Hull? sourceHull, Vector2 listenerPos, Hull? listenerHull, Submarine? submarine, int n = 1, bool ignoreDoors = false)
        {
            ...
            if (sourceHull == listenerHull)
            {
                ...
                results.Add(new PathfindingResult(true, dist, 0, 0, null, sourcePos, dist)); // Use sourcePos as intersection, cost=dist
                return results;
            }
            ...
            foreach (var gap in s_hullConnectivityGraph.Keys) { gScore[gap] = float.MaxValue; fScore[gap] = float.MaxValue; }
            ...
            {
                float initialGScore = Vector2.Distance(sourcePos, startGap.WorldPosition); 
                gScore[startGap] = initialGScore;
                fScore[startGap] = initialGScore + Heuristic(startGap, listenerPos); 
            ...
            while (openSet.Count > 0)
            {
                ...
                if (s_hullConnectivityGraph.TryGetValue(currentGap, out var intraHullNeighbors))
                {
                    foreach (var (neighborGap, baseDistance) in intraHullNeighbors)
                    {
                        ...
                        {
                            predecessors[neighborGap] = currentGap; gScore[neighborGap] = tentative_gScore;
                            fScore[neighborGap] = tentative_gScore + Heuristic(neighborGap, listenerPos); openSet.Enqueue(neighborGap, fScore[neighborGap]);
                        }
                    }
                }
                ...
                foreach (Hull adjacentHull in currentGap.linkedTo.OfType<Hull>())
                {
                    ...
                    foreach (Gap nextGap in adjacentHull.ConnectedGaps.Where(g => g != null && gScore.ContainsKey(g)))
                    {
                        ...
                        // Cost = CostToCurrent + PenaltyForExitingCurrent + DistanceToNext
                        ...
                        if (tentative_gScore < gScore[nextGap])
                        {
                            ...
                            fScore[nextGap] = tentative_gScore + Heuristic(nextGap, listenerPos); openSet.Enqueue(nextGap, fScore[nextGap]);
                        }
                    }
                }
            } // End A* loop
            ...
            foreach (var endpoint in potentialEndpoints)
            {
                ...
                if (addedFinalGaps.Add(finalGap))
                {
                    ...
                    if (path != null && path.Count > 0)
                    {
                        ...
                        int doorCount = 0; HashSet<Gap> doorsCounted = new HashSet<Gap>();
        ...
        private static (float totalDistance, Vector2 lastIntersection) CalculateAccuratePathDetails(Vector2 sourcePos, Vector2 listenerPos, List<Gap> path)
        {
            ...
            Vector2 lastCalculatedIntersection = sourcePos;
            for (int i = 0; i < path.Count; i++) 
            { 
                Gap currentGap = path[i]; 
                if (currentGap == null) continue; 
                Vector2 nextPos = (i + 1 < path.Count && path[i + 1] != null) ? path[i + 1].WorldPosition : listenerPos; Vector2 intersectionPoint = GetGapIntersectionPos(currentPos, nextPos, currentGap);
                totalDistance += Vector2.Distance(currentPos, intersectionPoint); 
                currentPos = intersectionPoint; 
                lastCalculatedIntersection = currentPos; 
            }
            totalDistance += Vector2.Distance(currentPos, listenerPos); return (totalDistance, lastCalculatedIntersection);
        }
    }
}
